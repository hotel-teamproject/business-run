...
exports.getMyReservations = async (req, res) => {
  try {
    ...
    const reservations = bookings.map(booking => ({
      _id: booking._id,
      reservationNumber: booking._id.toString().slice(-8).toUpperCase(),
      guestName: booking.guestName,
      hotelName: booking.hotelId?.name || '',
      roomName: booking.roomId?.name || '',
      startDate: booking.checkIn ? booking.checkIn.toISOString().split('T')[0] : '',
      endDate: booking.checkOut ? booking.checkOut.toISOString().split('T')[0] : '',
