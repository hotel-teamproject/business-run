...
const businessReviewRoutes = require('./reviews/businessReviewRoutes');
const businessStatsRoutes = require('./stats/businessStatsRoutes');
...
// MongoDB 연결
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27018/business-back', {
...
// 사업자 인증 라우트 (admin auth와 동일한 컨트롤러 사용)
// 주의: /api/business/auth는 /api/business보다 먼저 등록해야 함 (더 구체적인 경로 우선)
...
app.use('/api/business/statistics', businessStatsRoutes);
// inventory 라우트는 마지막에 등록 (경로가 /rooms/:roomId/inventory이므로)
app.use('/api/business', inventoryRoutes);
