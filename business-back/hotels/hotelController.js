...
exports.getMyHotels = async (req, res) => {
  try {
    ...
    // 각 호텔에 추가 정보 조회 (객실 수, 리뷰 수, 평균 평점)
    const hotelsWithStats = await Promise.all(hotels.map(async (hotel) => {
      ...
      hotelObj.reviewCount = reviewStats[0]?.count || 0;
      hotelObj.averageRating = reviewStats[0]?.avgRating || hotel.rating || 0;
      
      return hotelObj;
    }));
    
    res.json({ success: true, data: hotelsWithStats });
  } catch (error) {
